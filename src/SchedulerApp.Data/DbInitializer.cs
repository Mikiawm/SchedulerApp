...
namespace SchedulerApp.Data
{
    public class DbInitializer
    {
        public static void Initialize(SchedulerContext context)
        {
            ...
            if(!context.Happenings.Any())
            {
                var happenings = new Happening[]
                {
                    new Happening("Name1", DateTime.Now, DateTime.Now.AddDays(5)),
                    new Happening("Name2", DateTime.Now.AddDays(2), DateTime.Now.AddDays(5)),
                    new Happening("Name1", DateTime.Now.AddDays(3), DateTime.Now.AddDays(7)),
                    new Happening("Name1", DateTime.Now.AddDays(-5), DateTime.Now.AddDays(1)),
                    new Happening("Name1", DateTime.Now.AddDays(-3), DateTime.Now.AddDays(2)),
                    new Happening("Name1", DateTime.Now, DateTime.Now.AddDays(2))
                };
                foreach (Happening c in happenings)
                {
                    context.Happenings.Add(c);
                }
                context.SaveChanges();
            }
        }
